...
@Composable
fun PokemonDetailAbilityScore(attributes: Attributes, hp: Int, ac: Int) {
...
private fun AttributeRow(text: String, maxValue: Float, attribute: Int, color: Color, modifier: Modifier) {
    Row(modifier = modifier.padding(5.dp).width(400.dp).height(25.dp), horizontalArrangement = Arrangement.SpaceEvenly) {
        Text(text = text, fontSize = 20.sp, fontWeight = FontWeight.ExtraBold, modifier = Modifier.width(43.dp))
        LinearProgressIndicator(progress = attribute / maxValue, color = color, modifier = Modifier.fillMaxHeight())
        Text(text = attribute.toString(), fontSize = 20.sp, fontWeight = FontWeight.ExtraBold, modifier = Modifier.width(43.dp))
    }
}
